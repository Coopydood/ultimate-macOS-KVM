...
class Smbios:
    ...
    def _get_plist(self):
        ...
        print("   Q. Quit")
        print("   ")
        if os.path.exists(repoDir+"/boot/mnt/EFI/OC/config.plist"):
             p = (repoDir+"/boot/mnt/EFI/OC/config.plist")
             ultmosDetect = 1
        else:
            p = self.u.grab("Drag and drop your config.plist file: ")
            ultmosDetect = 0
        if p.lower() == "q":
            self.u.custom_quit()
        elif p.lower() == "m":
    ...
    def main(self):
        ...
        if not os.path.exists(repoDir+"/boot/mnt/EFI/OC/config.plist") and os.path.exists(repoDir+"/boot/OpenCore.qcow2"):
            clear()
            print("\n\n  "+color.BOLD+color.BLUE,"MOUNT ULTMOS OPENCORE?"+color.END,"")
            print("   Enables auto plist detection\n"+color.END)
            print("   Would you like to mount your ULTMOS OpenCore image?\n   Doing so allows you to automatically edit its SMBIOS\n   directly, including changing the serial number.\n\n"+color.END)
            print(color.BOLD+"   1. Mount (recommended)"+color.END)
            print("   2. Skip\n")
            ...
            if self.plist == None:
                ...
                clear()
        clear()
        print("\n\n  "+color.BOLD+color.BLUE,"GenSMBIOS"+color.END,"")
        print("   by",color.BOLD+"CorpNewt\n"+color.END)

        if ultmosDetect == 1:
            print(color.BOLD+"   TARGET:"+" ULTMOS "+color.BOLD+color.GREEN+"   VERIFIED"+color.END)
            print(color.BOLD+"     TYPE:"+color.END+" Built-In")
            #print(color.BOLD+"   TARGET:"+" ULTMOS"+color.BOLD+color.GREEN+"\n           VERIFIED"+color.END)
        else:
            print(color.BOLD+"   TARGET:"+" {}".format(os.path.basename(self.plist))+color.BOLD+color.RED+"   UNVERIFIED"+color.END)
            print(color.BOLD+"     TYPE:"+color.END+" {}".format(self.plist_type)+"")
        print("   ")
        #print("   1. Install/Update MacSerial")
        #print("   2. Select config.plist")
        print(color.BOLD+"   1. Generate SMBIOS"+color.END)
        print("      Start generation of a new SMBIOS,\n      including a new serial number.\n")
        print("   2. Generate UUID")
        print("   3. Generate ROM")
        print("   4. List Current SMBIOS")
        print("   5. Generate ROM With SMBIOS (Currently {})".format("Enabled" if self.gen_rom else "Disabled"))
        print("   ")
        print("   Q. Quit to Extras...")
        print("   ")
        menu = self.u.grab(color.BOLD+"Select> ").lower()
        if not len(menu):
            return
        if menu == "q":
            oldDir = os.curdir
